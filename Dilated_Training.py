...
def build_temporal_cond(batch, dev, n_frames=T_COND):
    if "tc_frames" not in batch:
        raise ValueError("CRITICAL BUG: 'tc_frames' missing from batch! Update your Dataset.py to yield historical frames.")
...
def train():
    ...
    sched =CosineAnnealingWarmRestarts(opt,T_0=100,T_mult=1,eta_min=MIN_LR)
...
class CorrDiffInference:
    ...
    def predict(self, coarse, var_map, topo, d2m, doy, hour, tc_frames):
        dev = self.dev
        coarse, var_map, topo = coarse.to(dev), var_map.to(dev), topo.to(dev)
        d2m, tc_frames = d2m.to(dev), tc_frames.to(dev)
        gf = torch.stack([doy.to(dev), hour.to(dev)], dim=1).float()

        xi = build_coarse_input(coarse, var_map)
        tp = expand_topo(topo)

        # Autocast only the regressor
        ...
        for _ in range(self.n_ens):
            # Pure Diffusion Output + Regressor
            s_log = ddim_sample(self.unet, mu, tc_frames, tp, gf,
                                d2m, var_map, self.edm_sched, dev, self.tc_cfg, 1.0) + mu
            
            # ── 1. DISABLE PHYSICS GUIDE ──
            # s_log = PhysicsGuide.apply(s_log, coarse, enforce_mass=True, enforce_dry=True)
            
            s_phys = torch.expm1(s_log.clamp(min=0.0))
            samples_phys.append(s_phys)
            
        mean_pred_phys = torch.stack(samples_phys).float().mean(0)
        mean_pred = torch.log1p(mean_pred_phys)

        # ── 2. DISABLE QDM FOR NOW ──
        # if self.qdm is not None:
        #     cm = self.qdm.cm.to(dev)
        #     co = self.qdm.co.to(dev)
        #     x_log = mean_pred.flatten().float()
        #     idx = torch.searchsorted(cm.contiguous(), x_log.contiguous()).clamp(0, self.qdm.n-1)
        #     calibrated_log = torch.clamp(co[idx] + (x_log - cm[idx]), min=0.0)
        #     mean_pred = calibrated_log.reshape(mean_pred.shape).to(mean_pred.dtype)
